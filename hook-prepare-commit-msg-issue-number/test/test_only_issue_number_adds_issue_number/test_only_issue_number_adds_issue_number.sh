...
debug 'Changing local editor to sed to mock user input'
...
info 'Committing file with issue number in branch name'
git commit || fail 'Test failed. Commit was not allowed, but was expected to be allowed'

info 'Checking commit log'
[ 0 -eq $(git log -1 | grep "$BRANCH_NAME" | wc -l) ] && fail 'Test failed: branch name was not added to file, but it should have'
